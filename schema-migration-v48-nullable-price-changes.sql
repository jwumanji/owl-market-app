-- Migration v48: preserve unavailable price-change metrics as NULL.
--
-- v47 owns the complete summary refresh definition. This migration upgrades
-- an already-deployed v47 function without duplicating its large body here.
-- It is idempotent and fails if the expected function shape is not present.
...
begin
  ...
  corrected_definition := replace(
    ...
    $from$coalesce(avg(chg_7d), 0) as chg_7d$from$,
    $to$avg(chg_7d) as chg_7d$to$
  );
  corrected_definition := replace(
    corrected_definition,
    $from$coalesce(avg(chg_30d), 0) as chg_30d$from$,
    $to$avg(chg_30d) as chg_30d$to$
  );
  corrected_definition := replace(
    corrected_definition,
    $from$'chg1d', coalesce(chg_1d, 0)$from$,
    $to$'chg1d', chg_1d$to$
  );
  corrected_definition := replace(
    corrected_definition,
    $from$'chg7d', coalesce(chg_7d, 0)$from$,
    $to$'chg7d', chg_7d$to$
  );
  corrected_definition := replace(
    corrected_definition,
    $from$'chg30d', coalesce(chg_30d, 0)$from$,
    $to$'chg30d', chg_30d$to$
  );
  corrected_definition := replace(
    corrected_definition,
    $from$round(coalesce(aggregates.chg_7d, 0), 1)$from$,
    $to$round(aggregates.chg_7d, 1)$to$
  );
  corrected_definition := replace(
    corrected_definition,
    $from$round(coalesce(aggregates.chg_30d, 0), 1)$from$,
    $to$round(aggregates.chg_30d, 1)$to$
  );
