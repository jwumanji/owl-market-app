...
// Searchable card list for the eBay audit page. SSR-rendered links work
// without JS; the filter box is the only client-stateful part.
export default function EbayAuditPicker({
  cards,
  selectedId,
}: {
  cards: PickerCard[];
          ...
          <ul>
            {filtered.map((c) => {
              ...
              return (
                <li key={c.id}>
                  <Link
                    href={`/dev/ebay-audit?card=${encodeURIComponent(c.id)}`}
