import { RARITY_META, type RarityData } from "./rarities-data";
import {
  gameResponsePayload,
...
// /rarities, /games/[game]/rarities, and GET /api/rarities so the page can be
// server-rendered without a client-side fetch of its own API route.
// ---------------------------------------------------------------------------

export class RaritiesLoadError extends Error {
  status: number;

  constructor(message: string, status: number) {
    super(message);
    this.name = "RaritiesLoadError";
    this.status = status;
  }
}

export type LoadedRarities = {
...
async function loadCatalogOnlyRarities(supabase: SupabaseServiceClient, gameId: string) {
  ...
  if (error) {
    throw new RaritiesLoadError(error.message, 500);
  }
  ...
  return results.filter((rarity) => rarity.cardCount > 0).sort((a, b) => b.cardCount - a.cardCount);
}
...
export async function loadRarities(options: { game?: string | null } = {}): Promise<LoadedRarities> {
  ...
  if (gameResult.error) {
    throw new RaritiesLoadError(gameResult.error.message, gameResult.error.status);
  }
  const { game } = gameResult;

  if (game.slug !== ONE_PIECE_DB_SLUG) {
    const summaryRows = await loadRaritySummaries(supabase, game.id, game.slug);
    const rarities = summaryRows ?? (await loadCatalogOnlyRarities(supabase, game.id));
