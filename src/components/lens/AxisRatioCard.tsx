...
export function AxisRatioValue({
  ...
  return (
    <div className={`mt-1 flex max-w-full min-w-0 flex-nowrap items-baseline gap-1 font-mono-2 font-bold tabular-nums ${valueSize}`}>
      <span className="shrink-0 text-[8px] uppercase tracking-wider text-ink-2">{firstLabel}</span>
      <span className={TONE_TEXT_CLASSES[tone]}>{firstValue}</span>
      <span className="shrink-0 text-ink-3">/</span>
      <span className="shrink-0 text-[8px] uppercase tracking-wider text-ink-2">{secondLabel}</span>
