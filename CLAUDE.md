...
`market_index_snapshots` carries **two set series that mean different populations — never mix them.** `entity_type='set'` is the official rollup: `index_value = sum(tcg_market)` over the full printed-set population, **promo-inclusive** (45.5% of the One Piece index is promo/event paper — `docs/investigations/value-ratio-population-audit.md`). `entity_type='set_baseline'` (`metric_version=2`, admitted by migration `20260728100000`) sums only the **booster-baseline** population defined in `src/lib/games/one-piece/booster-baseline.ts`, and feeds every Value Ratio surface. **A consumer reading `entity_type='set'` gets the promo-inclusive figure — choose deliberately; this exact ambiguity is how the polluted ranking shipped.** Do not build a third rollup. Natural key is `(game_id, entity_type, entity_key, snapshot_date)` — no region, no `metric_version`, so `region` is inert until both the function and the constraint change together. Access is **service-role only**; anon/authenticated are revoked.
