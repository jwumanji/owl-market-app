-- OWL Market — Moon Terminal sealed module
--
-- Implements docs/moon-terminal-sealed-spec.md §2.2.
--
-- ############################################################################
-- ## ALREADY APPLIED BY HAND on 2026-07-26, before this file was moved into  ##
-- ## supabase/migrations/. Its tracking row was inserted manually:           ##
-- ##                                                                         ##
-- ##   insert into supabase_migrations.schema_migrations (version, name)     ##
-- ##   values ('20260726143000', 'terminal_sealed')                          ##
-- ##   on conflict (version) do nothing;                                     ##
-- ##                                                                         ##
-- ## Do NOT run this again by hand. It is idempotent, so a stray `db push`   ##
-- ## re-run would not corrupt anything, but it WOULD re-execute the          ##
-- ## external_ref backfill at the bottom of section 1.                       ##
-- ############################################################################
--
-- HISTORY: originally written as schema-migration-v49-terminal-sealed.sql under
-- the root convention. That convention is frozen at v48 and the live convention
-- is timestamped files in supabase/migrations/, so this was renamed. The spec
-- called it "v46"; v46/v47/v48 were already taken (character price index,
-- character links, nullable price changes), all three applied.
--
-- See also 20260726150000_terminal_sealed_index_cleanup.sql, which removes two
-- indexes this migration added that duplicated
-- 20260714153000_sealed_product_tracking.sql.
--
