...
**Two set series exist as of migration `20260728100000` — they mean different populations.** `entity_type='set'` is the official, **promo-inclusive** figure (45.5% of the One Piece index is promo/event paper; `docs/investigations/value-ratio-population-audit.md`). `entity_type='set_baseline'` (`metric_version=2`) sums only the booster-baseline population (`src/lib/games/one-piece/booster-baseline.ts`) and is what every **Value Ratio** surface consumes. They are not comparable, and any future consumer reading `entity_type='set'` gets the promo-inclusive number — **choose deliberately.** This ambiguity is exactly how the polluted Value Ratio ranking shipped in Phase D.
...
- **`market_index_snapshots` holds two non-comparable set series** — `entity_type='set'` (official, promo-inclusive) and `entity_type='set_baseline'` (booster-baseline, feeds Value Ratio); see §2.2 before writing any query against it.
