-- Migration v47: support canonical identities and cards that feature multiple
-- characters while preserving cards.character_id as the primary relationship.
-- Idempotent: creates/backfills links and replaces the summary refresh function.
create table if not exists public.card_character_links (
...
create index if not exists idx_card_character_links_character
  on public.card_character_links (game_id, character_id, card_id);

create index if not exists idx_card_character_links_card
  on public.card_character_links (game_id, card_id);
...
begin
  if not exists (
    ...
    create policy "Public card character links are readable"
      on public.card_character_links for select
      using (true);
  end if;
end
$$;

grant select on public.card_character_links to anon, authenticated;
grant all on public.card_character_links to service_role;

insert into public.card_character_links (game_id, card_id, character_id, is_primary)
select cards.game_id, cards.id, cards.character_id, true
from public.cards
where cards.character_id is not null
on conflict (game_id, card_id, character_id)
  ...
  loop
    ...
    priced_cards as (
      select
        ...
        normalized_cards.image_url_preview,
        sets.code as set_code,
        price_stats.tcg_market,
        price_stats.market_avg,
        coalesce(price_stats.tcg_market, price_stats.market_avg) as effective_price,
        price_stats.chg_1d,
        price_stats.chg_7d,
        price_stats.chg_30d
      from normalized_cards
      ...
      where coalesce(price_stats.tcg_market, price_stats.market_avg) is not null
    ),
    aggregates as (
      select
        rarity_code,
    ...
    ranked_top_cards as (
      select
        ...
        row_number() over (
          ...
          order by priced_cards.effective_price desc nulls last, priced_cards.name
        ) as rank
      from priced_cards
    ),
    top_cards as (
      select
        ...
        jsonb_agg(
          jsonb_build_object(
            ...
            'imagePreview', image_url_preview
          )
          order by effective_price desc nulls last, name
        ) as top_cards
    ...
    with card_counts as (
      select links.character_id, count(*)::integer as card_count
      from public.card_character_links links
      join public.cards
        on cards.id = links.card_id
       and cards.game_id = links.game_id
      where links.game_id = game_row.id
      group by links.character_id
    ),
    priced_cards as (
      select
        links.character_id,
      ...
      where links.game_id = game_row.id
        and coalesce(price_stats.tcg_market, price_stats.market_avg) is not null
    ),
    aggregates as (
      select
        character_id,
