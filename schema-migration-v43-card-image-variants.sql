-- v43: Hosted card image variants
-- Safe to run after v42. This adds provenance + mirror status columns for
-- hosted thumbnail/preview/display card images, then refreshes the public page
-- summary cache so top-card payloads can expose preview-sized URLs.

do $$
begin
  if to_regclass('public.cards') is null then
    raise exception 'public.cards is required before running v43 card image variants';
  end if;
end;
$$;

alter table public.cards
  add column if not exists image_url_preview text,
  add column if not exists image_source_url text,
  add column if not exists image_storage_path text,
  add column if not exists image_mirror_status text not null default 'external',
...
begin
  if not exists (
    ...
    where conrelid = 'public.cards'::regclass
      and conname = 'cards_image_mirror_status_check'
  ) then
    alter table public.cards
      add constraint cards_image_mirror_status_check
      check (image_mirror_status in ('external', 'pending', 'mirrored', 'skipped', 'error'));
  end if;
end;
$$;

create index if not exists idx_cards_game_image_mirror_status
  on public.cards(game_id, image_mirror_status);

create index if not exists idx_cards_image_storage_path
  on public.cards(image_storage_path)
